...
class ParticipantesSorteo {
  ...
  obtenerParticipantes = async () => {
    devolverParticipante = async (id) => {
      ...
      let participante = await baseDeDatos.db
        .collection(config.COLECCION_PARTICIPANTES)
  ...
  devolverParticipantes = async () => {
    if (!baseDeDatos.conectado) return [];
    try {
      let participantes = await baseDeDatos.db
        .collection(config.COLECCION_PARTICIPANTES)
        .find({})
        .toArray();
      ...
      console.log(
        `No se pudo devolver a los participantes, motivo de error:  ${err.message}`
      );
      return [];
    }
  };
  ...
  ingresarParticipante = async (participante) => {
    try {
      if (!baseDeDatos.conectado) return {};
      await baseDeDatos.db
        .collection(config.COLECCION_PARTICIPANTES)
        .insertOne(participante);
      return participante;
    } catch (error) {
      console.log(
        `No se pudo agregar al participante, motivo de error:  ${error.message}`
      );
    }
  };
  ...
  modificarParticipante = async (participante, id) => {
    try {
      ...
      await baseDeDatos.db
        .collection(config.COLECCION_PARTICIPANTES)
        .updateOne({ _id: ObjectId(id) }, { $set: participante });
